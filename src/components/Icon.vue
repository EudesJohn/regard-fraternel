...
// Garde-fou XSS : v-html n'accepte QUE les icônes du registre statique ci-dessous
// (jamais de contenu utilisateur ou distant). Ce registre est la seule source.
const SAFE_ICON_NAMES = new Set([
  'users', 'briefcase', 'compass', 'scale', 'shield', 'handshake', 'sprout',
  'graduation', 'wheat', 'sunrise', 'heart', 'mapPin', 'phone', 'mail',
  'landmark', 'fileText', 'download', 'check', 'arrowRight', 'chevronLeft',
  'chevronRight', 'x', 'menu', 'zoomIn', 'target', 'send', 'facebook', 'rotate'
])
...
<template>
  <svg
    ...
    aria-hidden="true"
    v-html="(SAFE_ICON_NAMES.has(name) && paths[name]) || ''"
  />
</template>
